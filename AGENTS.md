...
| `config/quip-miner.{cpu,cuda}.toml` | Miner first-run config templates, forked from the quip-miner repo's docker templates with the testnet `faucet_url` filled in. Bind-mounted over `/app/quip-miner.docker.toml`; the entrypoint copies one to `data/config.toml` when that file doesn't exist. |
