...
| `[global].log_level`, `.node_log` | `[miner].log_level` | `log_level` promoted. `node_log` dropped: the miner logs to stdout and the stack merges every service into `data/logs/quip-node.log`. |
