...
- **Image tag vars** (`QUIP_MINER_TAG`, `QUIP_VALIDATOR_TAG`, `QUIP_DASHBOARD_TAG`, `QUIP_FAUCET_TAG`) are unset by default. `CHANNEL` names the tag instead, and every image publishes both `beta` and `stable`. Both channels run Aglais: `stable` is the released line, `beta` the prerelease line ahead of it. The validator is the exception, because its upstream `stable` tag still names the retired v0.2.2 build, so `docker-compose.yml` gives it an explicit Aglais tag. Set a tag var only to pin an exact version, and note that a stale pin holds the stack on an old build. `make show-channel` prints what you will actually pull.
